...
All the environments are registered in the `Go2_Nero_Gripper` extension. They start
...
import Go2_Nero_Gripper.tasks  # noqa: F401

# PLACEHOLDER: Extension template (do not remove this comment)
with contextlib.suppress(ImportError):
    import Go2_Nero_Gripper.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~Go2_Nero_Gripper.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from Go2_Nero_Gripper.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `Go2_Nero_Gripper` extension."""
    # Collect matching task specs first so we can enumerate presets in one pass.
    task_specs = [
        spec
        for spec in gym.registry.values()
        if "GO2-NERO" in spec.id and (args_cli.keyword is None or args_cli.keyword in spec.id)
    ]

    if args_cli.show_presets:
        from Go2_Nero_Gripper.utils.preset_cli import enumerate_task_presets
